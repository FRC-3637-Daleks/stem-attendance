...
export async function removeCheckins(category, count) {
  // Step 1: Get the IDs of the most recent N checkins for this category
  const { data, error } = await supabase
    .from('checkins')
    .select('id')
  ...
  if (error) { console.error('removeCheckins select error:', error); return 0 }
  if (!data || !data.length) { console.warn('No checkins found to remove'); return 0 }

  const ids = data.map(r => r.id)
  console.log(`Deleting ${ids.length} checkins:`, ids)

  // Step 2: Delete them one-by-one as a fallback if .in() has RLS issues
  let deleted = 0
  for (const id of ids) {
    const { error: delError } = await supabase
      ...
      .eq('id', id)
    if (delError) {
      console.error(`Failed to delete checkin ${id}:`, delError)
    } else {
      deleted++
    }
  }
  ...
  if (deleted > 0) {
    await addLog('removed', `${deleted} × ${category}`)
  }
  return deleted
}
